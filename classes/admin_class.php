...
/* -------------admin_password_change------------*/
	...
	public function add_new_task($data){
		...
		try{
			$add_task = $this->db->prepare("INSERT INTO task_info (t_location, t_description, t_start_time, 	t_end_time, t_user_id) VALUES (:x, :y, :z, :a, :b) ");
		...
		public function update_task_info($data, $Reserviour_id, $user_role){
			$task_title  = $this->test_form_input_data($data['task_title']);
			$task_description = $this->test_form_input_data($data['task_description']);
			$t_start_time = $this->test_form_input_data($data['t_start_time']);
			$t_end_time = $this->test_form_input_data($data['t_end_time']);
			...
			if($user_role == 1){
				...
				$sql = "SELECT * FROM task_info WHERE Reserviour_id='$Reserviour_id' ";
			...
			try{
				$update_task = $this->db->prepare("UPDATE task_info SET t_location = :x, t_description = :y, t_start_time = :z, t_end_time = :a, t_user_id = :b, status = :c WHERE Reserviour_id = :id ");
				...
				$update_task->bindparam(':id', $Reserviour_id);
